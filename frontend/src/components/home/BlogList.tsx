...
import useBlogsByCategory, {
  type BlogCategoryFilter,
  fetchBlogsByCategory,
  type Blog,
} from "@hooks/useBlogsByCategory";
...
export default function BlogList() {
  ...
  const { data, isLoading, isError, error, isPlaceholderData } =
    useBlogsByCategory({ category, page, q: searchText });
  ...
  return (
    <section className="p-2 mb-24">
      ...
      <div className="relative flex flex-wrap justify-center gap-2 my-10 sm:gap-4">
        {(["all", ...blogCategories] as BlogCategoryFilter[]).map(
          (cat, index) => (
            <button
              key={index}
              type="button"
              className={`relative px-5 py-1.5 text-gray-500 capitalize rounded-full cursor-pointer ${
